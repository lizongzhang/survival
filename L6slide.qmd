...
-   与参考组相比（sex=0为女性，sex=1为男性），男性发生type1结局（因黑色素瘤死亡）的亚分风险比为1.80。

-   男性发生该结局的风险是女性的1.8倍。p值小于0.05，说明性别的影响具有统计学意义。
...
-   每增加一岁，type1结局的亚分风险比增加1%（1.01倍），但p值为0.18，不具有统计学显著性（即年龄对type1结局风险的影响不显著）。

-   这可能因年龄范围较窄或竞争事件掩盖了效应。
...
$h_{01}(t)$：基线子分布危险率，依赖于时间 $t$和数据，但 crr()没有直接估计$h_{01}(t)$，需要通过非参数方法估计。
...
-   `survminer::ggcompetingrisks()`, `ggsurvfit::ggcumic()`
